...
import { getMockTestDetailsRequest, startTestRequest, clearStartTestState, setActiveTestId } from '../../Redux/Reducers/MockTestReducer';
import { RootState } from '../../Redux/Store';
import { normalize, verticalScale } from '../../Utils/Helpers/normalize';
import { ROUTES } from '../../Navigation/RouteNames';
import LinearGradient from 'react-native-linear-gradient';

type MockTestRulesScreenProps = {
    navigation: any;
    route: any;
};
...
const MockTestRulesScreen = ({ navigation, route }: MockTestRulesScreenProps) => {
    ...
    }, [dispatch, testId, testData]);

    useEffect(() => {
        if (isStarting && startTestResponse && !isLoading) {
            setIsStarting(false);
            navigation.replace(ROUTES.MOCK_TEST_QUESTION, {
                testId,
                duration,
                acceptedTerms: true,
                testData: resolvedTest,
            });
        }
    }, [isStarting, startTestResponse, isLoading, navigation, testId, duration, resolvedTest]);
            ...
            <ScrollView contentContainerStyle={[styles.scrollContent, { paddingBottom: Math.max(insets.bottom + verticalScale(96), verticalScale(120)) }]} showsVerticalScrollIndicator={false}>
                <LinearGradient colors={['#667EEA', '#764BA2']} start={{x: 0, y: 0}} end={{x: 1, y: 1}} style={styles.infoCard}>
                    <View style={styles.titleRow}>
                        ...
                        <Text style={[styles.priceText, {color: '#FFFFFF'}]}>{price > 0 ? `Rs. ${price}` : 'Free'}</Text>
                    </View>

                    <View style={styles.metaRow}>
                        <View style={[styles.metaChip, {backgroundColor: 'rgba(255,255,255,0.2)'}]}>
                            <Icon name="clock" size={12} color="#FFFFFF" />
                            <Text style={[styles.metaChipText, {color: '#FFFFFF'}]}>{duration} mins</Text>
                        </View>
                        <View style={[styles.metaChip, {backgroundColor: 'rgba(255,255,255,0.2)'}]}>
                            <Icon name="star" size={12} color="#FFFFFF" />
                            <Text style={[styles.metaChipText, {color: '#FFFFFF'}]}>Full Marks {fullMarks || '--'}</Text>
                        </View>
                    </View>

                    <View style={styles.metaRow}>
                        <View style={[styles.metaChipWide, {backgroundColor: 'rgba(255,255,255,0.2)'}]}>
                            <Icon name="minus-circle" size={12} color="#FFFFFF" />
                            <Text style={[styles.metaChipWideText, {color: '#FFFFFF'}]}>Negative Marking {negativeMarking} / question</Text>
                        </View>
                    </View>
                </LinearGradient>
                ...
                <Pressable
                    style={[(isLoading && (status === getMockTestDetailsRequest.type || isStarting)) ? styles.primaryButtonDisabled : null]}
                    onPress={handleContinue}
                    disabled={isLoading && (status === getMockTestDetailsRequest.type || isStarting)}
                >
                    <LinearGradient colors={['#667EEA', '#764BA2']} start={{x: 0, y: 0}} end={{x: 1, y: 1}} style={styles.primaryButton}>
