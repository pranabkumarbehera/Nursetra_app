...
const ensureLoginRedirectUrl = (paymentUrl: string) => {
    ...
    try {
        ...
        url.searchParams.set('redirect_url', 'https://www.nursetra.in/login');
        return url.toString();
    } catch {
        const hasQuery = paymentUrl.includes('?');
        const redirectParam = 'redirect_url=https%3A%2F%2Fwww.nursetra.in%2Flogin';
...
export function* startTestSaga(action: any): Generator<any, void, any> {
    const mockTestState = yield select((state: any) => state.MockTestReducer);
    const auth = yield select(getAuth);
    const header = {
        Accept: 'application/json',
        contenttype: 'application/json',
    ...
    try {
        const quizId = action.payload?.id || mockTestState?.activeTestId;
        const payload = {
            acceptedTerms: Boolean(action.payload?.acceptedTerms),
        };
        if (!quizId) {
            yield put(startTestFailure(undefined as any));
            Toast.show({ type: 'error', text1: 'Missing quiz id' });
            return;
        }
        const response = yield call(postApi, `student/quizzes/${quizId}/start`, payload, header);
