...
import { getProfileRequest } from '../Reducers/ProfileReducer';
...
export function* login_Saga(action: any): Generator<any, void, any> {
    ...
    try {
        ...
        if (token || response?.status === 200 || response?.status === 201) {
            ...
            if (token) {
                ...
                if (action.payload?.rememberMe) {
                    yield call(AsyncStorage.setItem, constants.REMEMBER_PASSWORD, 'true');
                    yield call(AsyncStorage.setItem, constants.SAVED_EMAIL, action.payload.email);
                    yield call(AsyncStorage.setItem, constants.SAVED_PASSWORD, action.payload.password);
                } else {
                    yield call(AsyncStorage.setItem, constants.REMEMBER_PASSWORD, 'false');
                    yield call(AsyncStorage.removeItem, constants.SAVED_EMAIL);
                    yield call(AsyncStorage.removeItem, constants.SAVED_PASSWORD);
                }
        ...
        const errMsg = error?.response?.data?.message || error?.response?.data?.error || error?.message || '!Oops something went wrong';
...
export function* refreshSaga(action: any): Generator<any, void, any> {
    const header = {
        Accept: 'application/json',
        contenttype: 'application/json',
    };
    const refreshToken = action?.payload?.refreshToken || action?.payload || '';
    ...
    try {
        ...
        const response = yield call(postApi, 'auth/refresh', { refreshToken }, header);
        if (response?.data?.success === true || response?.status === 200 || response?.status === 201) {
            ...
            if (nextAccessToken) {
                yield call(AsyncStorage.setItem, constants.TOKEN, nextAccessToken);
                yield put(tokenSuccess(nextAccessToken));
            }
            if (nextRefreshToken) {
                yield call(AsyncStorage.setItem, constants.REFRESH_TOKEN, nextRefreshToken);
            }

            yield put(refreshSuccess(response?.data));
        } else {
            yield put(refreshFailure(response?.data));
        }
    } catch (error: any) {
...
const AuthSaga = [
    ...
    takeLatest(refreshRequest.type, refreshSaga),
